...
CMatrix_arma CMatrix_arma::diag_mat()
{
    CMatrix_arma M(getnumrows());
    for (int i=0; i<getnumrows(); i++)
    {
        M(i,i) = matr(i,i);
    }
...
CMatrix_arma diag_mat(CMatrix_arma &M)
{
    return M.diag_mat();
}

CMatrix_arma CMatrix_arma::max(double &x)
{
    CMatrix_arma M(numcols);
    for (int i=0; i<numrows; i++)
    for (int j=0; j<numcols; j++)
    {
        M(i,j) = std::max(x,matr(i,j));
    }
    return M;
}

CMatrix_arma CMatrix_arma::min(double &x)
{
    CMatrix_arma M(numcols);
    for (int i=0; i<numrows; i++)
    for (int j=0; j<numcols; j++)
    {
        M(i,j) = std::min(x,matr(i,j));
    }
    return M;
}

CMatrix_arma max(CMatrix_arma &M, double &x) {return M.max(x);}
CMatrix_arma min(CMatrix_arma &M, double &x) {return M.min(x);}
